...
public class ValidationTest {
    ...
    public void testValidateItemsBeanValidationResultStringMapOfStringTBoolean() {
        ...
        validate(result -> Validation.validateItems(result, MY_FIELD, map3, true)).contains(Q_FIELD)
                        .contains("value for null-value").contains(NULL_MSG2);
        ...
        validate(result -> Validation.validateItems(result, MY_FIELD, map4, true)).contains(Q_FIELD).contains("key")
                        .doesNotContain("value for").contains(PfConcept.IS_NULL);
        validate(result -> Validation.validateItems(result, MY_FIELD, map4, false)).contains(Q_FIELD).contains("key")
                        .doesNotContain("value for").contains(PfConcept.IS_NULL);
    }

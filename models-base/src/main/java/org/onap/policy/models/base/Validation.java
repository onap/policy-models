...
public final class Validation {
    ...
    /**
     * Generates a function to validate that an object reference is not {@code null}.
     *
     * @param fieldName field name
     * @return a function to validate that an object reference is not {@code null}
     */
    public static <T> Function<T, ValidationResult> refNotNull(String fieldName) {
        return value -> {
            if (value == null) {
                ObjectValidationResult result = new ObjectValidationResult(fieldName, value);
                result.validateNotNull();
                return result;
            }
    ...
    public static <T extends Validated> void validateItemsNotNull(BeanValidationResult result, String subName,
        ...
        for (T item: subObject) {
            ...
            if (item != null) {
                result.addResult(item.validate(subName));
            }
        }
    }
    ...
    public static <T extends Validated> void validateItems(BeanValidationResult result, String subName,
                    Collection<T> subObject) {
        if (subObject == null) {
            return;
        }

        for (T item : subObject) {
            validateItem(result, subName, item, true);
        }
    }
    ...
    public static <T extends Validated> void validateItems(BeanValidationResult output, String subName,
        ...
        for (Entry<String, T> entry : subObject.entrySet()) {
            ...
            validateItem(result, "value for " + entry.getKey(), entry.getValue(), true);
        }

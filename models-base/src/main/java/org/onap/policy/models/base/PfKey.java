...
public abstract class PfKey extends PfConcept {
    ...
    /**
     * Validates the key, ensuring that the key is not null, though the version may be
     * "NULL" as long as the name is not.
     */
    @Override
    public ValidationResult validateNotNull(@NonNull final String fieldName) {
        if (isNullKey()) {

...
public class PdpStatisticsProvider {
     ...
     */
    public PdpStatistics deletePdpStatistics(@NonNull final PfDao dao, @NonNull final String name,
            final Date timestamp) {

        PfTimestampKey pdpStatisticsKey = new PfTimestampKey(name, PfKey.NULL_KEY_VERSION, timestamp);

        JpaPdpStatistics jpaDeletePdpStatistics = dao.get(JpaPdpStatistics.class, pdpStatisticsKey);

        if (jpaDeletePdpStatistics == null) {
            String errorMessage =
                    "delete of PDP group \"" + pdpStatisticsKey.getId() + "\" failed, PDP group does not exist";
            throw new PfModelRuntimeException(Response.Status.BAD_REQUEST, errorMessage);
        }

        dao.delete(jpaDeletePdpStatistics);

        return jpaDeletePdpStatistics.toAuthorative();
    }

    /**
     * Convert JPA PDP statictics list to an authorative PDP statictics list.
     *
    ...
    private List<PdpStatistics> asPdpStatisticsList(List<JpaPdpStatistics> jpaPdpStatisticsList) {
        List<PdpStatistics> pdpStatisticsList = new ArrayList<>(jpaPdpStatisticsList.size());

        for (JpaPdpStatistics jpaPdpStatistics : jpaPdpStatisticsList) {
            pdpStatisticsList.add(jpaPdpStatistics.toAuthorative());
        }

        return pdpStatisticsList;
    }
}

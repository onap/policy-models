...
public final class AaiManager {
     ...
     * @param password the password for authentication
     * @param requestId the UUID of the request
     * @param pnfId the PNF identifier
     * @return HashMap of PNF properties
     */
    public Map<String, String> getPnf(String url, String username, String password, UUID requestId, String pnfId) {
        String urlGet = url + PNF_URL + pnfId + AAI_DEPTH_SUFFIX;
        String responseGet = getStringQuery(urlGet, username, password, requestId, pnfId);
        if (responseGet == null) {
            return null;
        }
        try {
            Map<String, String> pnfParams = CODER.decode(responseGet, Map.class);

...
public class ControlLoopPolicyBuilderImpl implements ControlLoopPolicyBuilder {
    ...
    public Policy setPolicyForPolicyResult(PolicyParam policyParam, PolicyResult... results) throws BuilderException {
        ...
        //
        Policy newPolicy = new Policy(PolicyParam.builder().id(UUID.randomUUID().toString()).name(policyParam.getName())
            .description(policyParam.getDescription()).actor(policyParam.getActor()).payload(policyParam.getPayload())
            .target(policyParam.getTarget()).recipe(policyParam.getRecipe()).retries(policyParam.getRetries())
            .timeout(policyParam.getTimeout()).build());
        //
        // Connect the results
        //

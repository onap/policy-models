...
public class CdsSimulator {
     ...
     * @param requestedResponseDelayInMillis time for the request to be processed
     */
    public CdsSimulator(String host, int port, String resourceLocation, int countOfSuccesfulEvents,
        long requestedResponseDelayInMillis) {
        this.port = port;
                ...
                return new StreamObserver<ExecutionServiceInput>() {
                    ...
                    public void onNext(final ExecutionServiceInput executionServiceInput) {
                        ...
                        try {
                            ...
                            long timeLapsedSinceRequest = System.currentTimeMillis() - requestReceivedTime;
                            if (timeLapsedSinceRequest < requestedResponseDelayInMillis) {
                                long leftToSleep =  requestedResponseDelayInMillis - System.currentTimeMillis()
                                    + requestReceivedTime;
                                TimeUnit.MILLISECONDS.sleep(leftToSleep);
                            }

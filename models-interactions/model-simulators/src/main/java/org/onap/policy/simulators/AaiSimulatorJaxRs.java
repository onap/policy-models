...
public class AaiSimulatorJaxRs {
    ...
    public String aaiPutQuery(final String req) throws IOException {
        String resourceLocation = Registry.getOrDefault(this.getClass().getName() + "RESOURCE_LOCATION", String.class,
            "org/onap/policy/simulators/aai/");
        return ResourceUtils.getResourceAsString(resourceLocation + "AaiCqResponse.json");
    }
     ...
     */
    @GET
    @Path("/{version:v16|v21}/network/pnfs/pnf/pnf-id={pnfId}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces("application/json")
    public Response aaiGetPnfUsingPnfId(@PathParam("pnfId") final String pnfId) throws IOException {
     ...
     */
    @GET
    @Path("/{version:v16|v21}/network/generic-vnfs/generic-vnf/vnf-name={vnfName}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces("application/json")
    public Response aaiGetVnfUsingVnfName(@PathParam("vnfName") final String vnfName) throws IOException {
    ...
    private Response getResponse(final String expectedFileName, final String defaultFileName) {
        ...
        var responseString = ResourceUtils.getResourceAsString(resourceLocation + expectedFileName + ".json");
        if (null != responseString) {
            return Response.ok(responseString).build();
        } else {
            responseString = ResourceUtils.getResourceAsString(resourceLocation + defaultFileName + ".json");
            return Response.status(Response.Status.NOT_FOUND).entity(responseString).build();
        }
    }

    private String getResourceLocation() {
        return Registry.getOrDefault(this.getClass().getName() + "RESOURCE_LOCATION", String.class,
            "org/onap/policy/simulators/aai/");
    }
}

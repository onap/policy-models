...
public abstract class OperationPartial implements Operation {
     ...
     * @param adorn a function that "adorns" the future, possible adding onto its
     *        pipeline. Returns the adorned future
     * @return a array of futures, possibly zero-length. If the array is of size one, then
     *         that one item should be returned instead of the controller
     */
    ...
    /**
     * Performs a sequence of tasks, stopping if a task fails. A given tasks future is not
     * created until the previous task completes. The pipeline returns the outcome of the
     * last task executed.
     *

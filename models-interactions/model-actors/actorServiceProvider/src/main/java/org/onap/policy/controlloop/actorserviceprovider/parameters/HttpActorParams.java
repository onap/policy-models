...
public class HttpActorParams {
    ...
    /**
     * Extracts sub parameters from these parameters, for a given sub item.
     *
     * @param name name of the item containing these parameters
     * @return a function to extract sub parameters from these parameters, for a given sub
     *         item
     */
    public Function<String, Map<String, Object>> makeOperationParameters(String name) {
        ...
        return subName -> {
            String subpath = path.get(subName);
            if (subpath == null) {
                return null;
            }

            subparams.setPath(subpath);
            return Util.translateToMap(name + "." + subName, subparams);
        };
    }

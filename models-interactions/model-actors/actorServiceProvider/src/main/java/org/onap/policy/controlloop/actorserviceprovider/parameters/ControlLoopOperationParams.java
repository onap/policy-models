...
public class ControlLoopOperationParams {
    ...
    public CompletableFuture<ControlLoopOperation> start() {
        ...
        try {
            ...
            stopPipeline();
            // TODO should we re-throw after stopping the pipeline instead of returning a
            // future?
            return future;
        }
    }
    ...
    /**
     * Handles an exception by logging the exception and invoking the callbacks.
     *
     * @param exception exception to be handled
     * @return a completed future containing the outcome
     */
    private CompletableFuture<ControlLoopOperation> handleException(Exception exception) {
        ...
        if (pipelineController == null) {
            invokeCallbacks(operation);
        } else {
            pipelineController.doIfRunning(() -> invokeCallbacks(operation));
        }
    ...
    /**
     * Invokes the "started" and "completed" callbacks.
     *
     * @param operation operation to be passed to the callbacks
     */
    private void invokeCallbacks(ControlLoopOperation operation) {

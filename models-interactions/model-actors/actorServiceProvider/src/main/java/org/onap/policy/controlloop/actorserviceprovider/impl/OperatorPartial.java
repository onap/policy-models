...
public abstract class OperatorPartial extends ConfigurablePartial<Map<String, Object>> implements Operator {
    ...
    private Function<ControlLoopOperation, CompletableFuture<ControlLoopOperation>> handleFailure(
        ...
        return outcome -> {
            ...
            if (outcome != null && isSuccess(outcome)) {
                logger.trace("{}: no failure", getFullName());
    ...
    private CompletableFuture<ControlLoopOperation> startOperationAttempt(ControlLoopOperationParams params,
        ...
        CompletableFuture<ControlLoopOperation> future = startOperationOnce2(params, attempt);
    ...
    private CompletableFuture<ControlLoopOperation> startOperationOnce2(ControlLoopOperationParams params,
                    int attempt) {

        logger.info("{}: start low-level operation for {}", getFullName(), params.getRequestId());
        ...
        CompletableFuture<ControlLoopOperation> future2 =
            firstFuture.thenComposeAsync(verifyRunning(controller), executor)
    ...
    private Function<ControlLoopOperation, ControlLoopOperation> setRetryFlag(ControlLoopOperationParams params,
        ...
        return operation -> {
            if (operation != null && !isActorFailed(operation)) {
                ...
                logger.trace("{}: no retry needed for {}", getFullName(), params.getRequestId());
                return operation;
            }
            ...
            logger.trace("{}: setting retry flag {}", getFullName(), oper2.getOutcome());
            return oper2;
        };
    }
    ...
    private Function<ControlLoopOperation, CompletableFuture<ControlLoopOperation>> retryOnFailure(
        ...
        return operation -> {
            if (!isActorFailed(operation)) {
                // wrong type or wrong operation - just leave it as is
                logger.trace("{}: not retrying", getFullName());
    ...
    protected Function<ControlLoopOperation, CompletableFuture<ControlLoopOperation>> onSuccess(
        ...
        return operation -> {
            ...
            if (operation == null) {
                logger.trace("{}: null outcome - discarding next task", getFullName());
                ControlLoopOperation outcome = params.makeOutcome();
                outcome.setOutcome(OUTCOME_FAILURE);
                return CompletableFuture.completedFuture(outcome);

            } else if (isSuccess(operation)) {
                logger.trace("{}: success - starting next task", getFullName());
                return nextStep.apply(operation);

            } else {
                logger.trace("{}: failure - discarding next task", getFullName());
                return CompletableFuture.completedFuture(operation);
            }
        };
    }
    ...
    protected <T> Function<T, CompletableFuture<T>> verifyRunning(
                    PipelineControllerFuture<ControlLoopOperation> controller) {

        return value -> {
            boolean running = controller.isRunning();
            logger.trace("{}: verify running {}", getFullName(), running);
    ...
    protected ControlLoopOperation setOutcome(ControlLoopOperation operation, PolicyResult result) {
        logger.trace("{}: set outcome {}", getFullName(), result);

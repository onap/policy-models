...
public class VfModuleCreateTest extends BasicSoOperation {
    ...
    private static final Integer VF_COUNT = 10;
    ...
    public void testStartPreprocessorAsync() throws Exception {
        ...
        params.getContext().setProperty(VfModuleCreate.CONTEXT_KEY_VF_COUNT, 20);
    ...
    @Test
    public void testStoreAndRunGuard() throws Exception {
        // remove CQ data so it's forced to query
        context.removeProperty(AaiCqResponse.CONTEXT_KEY);
        ...
        oper.startPreprocessorAsync();
        provideCqResponse(makeCqResponse());
        assertTrue(executor.runAll(100));

        Integer vfcount = context.getProperty(VfModuleCreate.CONTEXT_KEY_VF_COUNT);
        assertEquals(VF_COUNT, vfcount);
    }

    @Test
    public void testVerifyNotNull() throws Exception {
        // remove CQ data so it's forced to query
        context.removeProperty(AaiCqResponse.CONTEXT_KEY);

        // null field in target
        params.getTarget().setModelInvariantId(null);
        ...
        assertThatThrownBy(() -> future2.get()).hasCauseInstanceOf(IllegalArgumentException.class);
    }

    @Test
    public void testMakeGuardPayload() {
        final int origCount = 30;
        params.getContext().setProperty(VfModuleCreate.CONTEXT_KEY_VF_COUNT, origCount);
    ...
    public void testStartOperationAsync_testSuccessfulCompletion() throws Exception {
        ...
        Integer newCount = (Integer) params.getContext().getProperty(VfModuleCreate.CONTEXT_KEY_VF_COUNT);
        assertEquals(origCount + 1, newCount.intValue());
    }
    ...
    private AaiCqResponse makeCqResponse() {

...
public class VfModuleDeleteTest extends BasicSoOperation {
    ...
    public void test() throws Exception {
        BusTopicParams pbus = BusTopicParams.builder()
                        .clientName("my-client")
                        .hostname("localhost")
                        .port(6969)
                        .build();
        HttpClient client = HttpClientFactoryInstance.getClientFactory().build(pbus);

        Entity<String> entity = Entity.entity("{}", MediaType.APPLICATION_JSON);

        client.getWebTarget().path("xyz").request().method("DELETE", entity);
    }

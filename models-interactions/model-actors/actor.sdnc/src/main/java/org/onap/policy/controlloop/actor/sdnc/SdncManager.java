...
public abstract class SdncManager extends OperationManagerPartial {
    ...
    @Override
    public <T> CompletableFuture<PolicyResult> start(CompletableFuture<T> future, ClosedLoopOperationParams params) {
        if (!isAlive()) {
        ...
        return future.thenApplyAsync(unused -> doRequest(request), params.getExecutor());
    }

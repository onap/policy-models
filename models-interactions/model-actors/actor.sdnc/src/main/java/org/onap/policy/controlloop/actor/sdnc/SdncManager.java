...
public abstract class SdncManager extends OperationManagerPartial {
    ...
    @Override
    protected <T> CompletableFuture<PolicyResult> appendOperation(ClosedLoopOperationParams params,
                    CompletableFuture<T> future, OperationTracker tracker) {

        SdncRequest request = constructRequest(params.getContext().getEvent());

        return future.thenCompose(tracker::verifyRunning).thenApplyAsync(unused -> doRequest(request),
                        params.getExecutor());
    }

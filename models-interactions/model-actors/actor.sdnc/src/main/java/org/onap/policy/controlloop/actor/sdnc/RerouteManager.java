...
public class RerouteManager extends SdncManager {
    private static final Logger logger = LoggerFactory.getLogger(RerouteManager.class);
    ...
    protected SdncRequest constructRequest(VirtualControlLoopEvent onset) {
        ...
        if (StringUtils.isBlank(serviceInstance)) {
            // TODO stick this message into an exception or into the operation output?
            logger.info("{}: missing enrichment data, service-instance-id", onset.getRequestId());
            return null;
        }
        ...
        if (StringUtils.isBlank(networkId)) {
            logger.info("{}: missing enrichment data, network-id", onset.getRequestId());
            return null;
        }

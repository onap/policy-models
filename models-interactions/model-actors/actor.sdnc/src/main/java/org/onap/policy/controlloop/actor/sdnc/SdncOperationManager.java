...
public abstract class SdncOperationManager extends OperationManagerPartial {
    ...
    protected void doConfigure(HttpParams parameters) {
        ValidationResult result = parameters.validate(getFullName());
        if (!result.isValid()) {
            throw new IllegalArgumentException(result.getResult());
        }

        client = HttpClientFactoryInstance.getClientFactory().get(parameters.getClientName());

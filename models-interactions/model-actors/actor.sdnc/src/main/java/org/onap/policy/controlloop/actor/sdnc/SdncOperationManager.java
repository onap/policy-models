...
import org.onap.policy.controlloop.actorserviceprovider.impl.OperationManagerPartial;
 ...
 */
public abstract class SdncOperationManager extends OperationManagerPartial {
    private static final Logger logger = LoggerFactory.getLogger(SdncOperationManager.class);
     ...
     */
    public SdncOperationManager(String actorName, String name) {
    ...
    @Override
    protected <T> CompletableFuture<OperationOutcome> doAppendOperation(ClosedLoopOperationParams params,
                    CompletableFuture<T> future) {

        final Executor executor = params.getExecutor();
        final SdncRequest request = constructRequest(params.getContext().getEvent());

        return future.thenApplyAsync(invokeStartCallback(params), executor)
                        .thenApplyAsync(unused -> doRequest(request), executor);
    }
     ...
     */
    private OperationOutcome doRequest(SdncRequest sdncRequest) {
        ...
        if (response.getResponseOutput() != null && !"200".equals(response.getResponseOutput().getResponseCode())) {
            ...
            return new OperationOutcome(PolicyResult.FAILURE);
        }

        return new OperationOutcome(PolicyResult.SUCCESS);
    }
}

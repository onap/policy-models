...
public class GrpcOperation extends OperationPartial {
    ...
    public CdsProcessorGrpcClient getClient() {
        return config.getClient();
    }

    public String getCdsStatus() {
        return config.getCdsStatus().get();
    }
    ...
    protected OperationOutcome doOperation(int attempt, OperationOutcome operation) {
        CdsResponse response = null;
        Optional<ExecutionServiceInput> optionalRequest = constructRequest(params);

        if (optionalRequest.isPresent()) {
            response = sendRequestToCds(optionalRequest.get());
        }
        setOutcome(operation, response);
        return operation;
    }
    ...
    public Optional<ExecutionServiceInput> constructRequest(ControlLoopOperationParams params) {
        ...
        // able to move it out.
        Map<String, String> payload = params.getPayload();
     ...
     */
    public CdsResponse sendRequestToCds(ExecutionServiceInput executionServiceInput) {
        try {
            logger.trace("Start CdsActorServiceProvider.executeCdsBlueprintProcessor {}.", executionServiceInput);
            // TO-DO: Handle requests asynchronously once the callback support is added to
            // actors.
            CountDownLatch countDownLatch = config.getClient().sendRequest(executionServiceInput);
            boolean status = countDownLatch.await(config.getTimeoutMs(), TimeUnit.MILLISECONDS);
            if (!status) {
                config.getCdsStatus().compareAndSet(null, CdsActorConstants.TIMED_OUT);
            }
            ...
            config.getCdsStatus().compareAndSet(null, CdsActorConstants.INTERRUPTED);
     ...
     * @param response cds response
     * @return the updated operation
     */
    public OperationOutcome setOutcome(OperationOutcome outcome, CdsResponse response) {

        switch (response.getStatus()) {
            case CdsActorConstants.SUCCESS:
                setOutcome(outcome, PolicyResult.SUCCESS);
                break;
            case CdsActorConstants.TIMED_OUT:
                setOutcome(outcome, PolicyResult.FAILURE_TIMEOUT);
                break;
            case CdsActorConstants.INTERRUPTED:
            case CdsActorConstants.ERROR:
                setOutcome(outcome, PolicyResult.FAILURE_EXCEPTION);
                break;
            default:
                setOutcome(outcome, PolicyResult.FAILURE);
                break;
        }
        return outcome;
    }

...
public class BasicBidirectionalTopicOperation {
    ...
    @Captor
    protected ArgumentCaptor<BiConsumer<String, StandardCoderObject>> listener;
    ...
    /**
     * Provides a response to the topic {@link #listener}.
     *
     * @param response response to be provided
     */
    protected void provideResponse(String response) {
        try {
            StandardCoderObject sco = coder.decode(response, StandardCoderObject.class);
            listener.getValue().accept(response, sco);

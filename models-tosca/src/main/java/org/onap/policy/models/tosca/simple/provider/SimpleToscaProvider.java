...
public class SimpleToscaProvider {
    ...
    public JpaToscaServiceTemplate updatePolicies(@NonNull final PfDao dao,
        ...
        EntityTree tree = makeTree(dbServiceTemplate);

        for (JpaToscaPolicy policy : serviceTemplate.getTopologyTemplate().getPolicies().getAll(null)) {
            verifyPolicyTypeForPolicy(tree, dbServiceTemplate.getPolicyTypes(), policy);
     ...
     *
     * @param tree tree containing policy types
     * @param policyTypes the available policy types in the service template
     * @param policy the policy to check the policy type for
     */
    private void verifyPolicyTypeForPolicy(EntityTree tree, final JpaToscaPolicyTypes policyTypes,
                    final JpaToscaPolicy policy) {
        if (policyTypes == null) {
        ...
        var type = tree.get(new EntityKey(NodeType.POLICY_TYPE, policy.getType()));

        if (type == null) {
            String errorMessage =
                POLICY_TYPE + policy.getType().getId() + " for policy " + policy.getId() + " does not exist";
            throw new PfModelRuntimeException(Response.Status.NOT_ACCEPTABLE, errorMessage);
        }
    }
}

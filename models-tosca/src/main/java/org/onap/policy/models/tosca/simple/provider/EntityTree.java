...
public class EntityTree {
    ...
    private <T> T getOptVersion(EntityKey key, Map<EntityKey, T> map) {
        ...
        return map.get(key.withNullVersion());
    }
    ...
    public Set<EntityKey> getUses(EntityKey key, NodeType...types) {
        return getRef(key, references, types);
    }
    ...
    public Set<EntityKey> getUsedBy(EntityKey key, NodeType...types) {
        return getRef(key, referencedBy, types);
    }

    private Set<EntityKey> getRef(EntityKey key, Map<EntityKey, Set<EntityKey>> ref, NodeType[] types) {
        var result = getOptVersion(key, ref);
        if (result == null) {

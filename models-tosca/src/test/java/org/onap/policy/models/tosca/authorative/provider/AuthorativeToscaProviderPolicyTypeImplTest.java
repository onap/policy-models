...
public class AuthorativeToscaProviderPolicyTypeImplTest {
    ...
    public void testPolicyTypeImplDelete() throws Exception {
        ...
        assertThatThrownBy(() -> {
            authorativeToscaProvider.deletePolicyTypeImpl(pfDao, null, null);
        }).hasMessageMatching("^name is marked .*on.*ull but is null$");

        assertThatThrownBy(() -> {
            authorativeToscaProvider.deletePolicyTypeImpl(pfDao, "name", null);
        }).hasMessageMatching("^version is marked .*on.*ull but is null$");

        assertThatThrownBy(() -> {
            authorativeToscaProvider.deletePolicyTypeImpl(pfDao, "dummyname", "1.0.1");
        }).hasMessageEndingWith("policy type impl not found in the database");

...
public class PolicyAuditProvider {
     ...
     */
    public void createAudits(@NonNull PfDao dao, @NonNull final List<PolicyAudit> audits) {
        dao.createCollection(audits.stream().map(JpaPolicyAudit::new).collect(Collectors.toList()));
    }
     ...
     */
    public List<PolicyAudit> getPolicyAudits(@NonNull PfDao dao, @NonNull Integer numRecords) {
        numRecords = numRecords > DEFAULT_MAX_RECORDS ? DEFAULT_MAX_RECORDS : numRecords;

        // @formatter:off
        return dao.getAll(JpaPolicyAudit.class, numRecords)
                .stream()
                .sorted((a1, a2) -> Long.valueOf(a2.getTimeStamp().getTime())
                        .compareTo(Long.valueOf(a1.getTimeStamp().getTime())))
     ...
     * @return list of {@link PolicyAudit} records
     */
    public List<PolicyAudit> getPolicyAudits(@NonNull PfDao dao, @NonNull AuditFilter auditFilter,
        ...
        if (auditFilter.getFromDate() == null) {
            auditFilter.setFromDate(Instant.EPOCH);
        }

        return dao
                .getFiltered(JpaPolicyAudit.class, auditFilter.getName(), auditFilter.getVersion(),
                        auditFilter.getFromDate(), Instant.now(), filter, DESCENDING_ORDER, numRecords)

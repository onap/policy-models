...
public class JpaPolicyAudit extends PfConcept implements PfAuthorative<PolicyAudit> {
    ...
    public BeanValidationResult validate(@NonNull String fieldName) {
        ...
        if (PfKey.NULL_KEY_NAME.equals(key.getName())) {
            addResult(result, "policy name", key.getName(), Validated.IS_NULL);
        }

        if (PfKey.NULL_KEY_VERSION.equals(key.getVersion())) {
            addResult(result, "policy version", key.getVersion(), Validated.IS_NULL);
        }

        if (action == null) {
            addResult(result, "policy action", action, Validated.IS_NULL);
        }

        if (timeStamp == null) {
            addResult(result, "timeStamp", timeStamp, Validated.IS_NULL);
        }
